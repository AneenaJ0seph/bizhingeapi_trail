...
class CartController extends GetxController {
  ...
  Future<void> checkout() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        Get.snackbar("Error", "Failed to place the order. ${response.statusCode}");
      }
    } catch (e) {
      Get.snackbar("Error", "An error occurred: $e");
    } finally {
  ...
  Future<void> updateAddress(String newAddress) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        Get.snackbar('Error', 'Failed to update address. ${response.statusCode}');
      }
    } catch (e) {
      Get.snackbar('Error', 'An error occurred: $e');
    } finally {
  ...
  Future<void> buyNow(Product product) async {
    ...
    final orderData = {
      "business_user": null,
      "total_price": (product.price ?? 0.0) * (product.minimumOrderQuantity ?? 1),
      "billing_address": address.value,
      "status": "Processing",
      "order_type": "Online",
      "order_products": [
